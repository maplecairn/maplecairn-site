...
One thing to know upfront. The hard part of this isn't the technical steps. The technical steps are well-documented and AI tools handle most of them. The hard part is scope discipline, picking a small enough v1, and shipping something you're slightly embarrassed by. I'll come back to that at the end.
...
The single most useful tool here is [namechk.com](https://namechk.com). Type a candidate name, it checks availability across all major social platforms and TLDs simultaneously. Saves you from picking a name only to discover the .ca is taken or the Instagram handle is gone.
...
> **What's Next.js?**
> A web framework built on top of React. Handles routing (which URL shows which page), server-side rendering, and a bunch of optimization stuff out of the box. It's what most modern websites are built with in 2026. The reason we picked it: pairs natively with Vercel hosting and has the smoothest deploy story.
...
> **What's auto-deploy?**
> Vercel watches your GitHub repo. Every time you push new code to your main branch, Vercel automatically builds and deploys it within 30-60 seconds. No FTP, no manual upload, no "click here to deploy." Save a file → git push → it's live. This is one of the genuinely magical parts of modern web hosting.
...
> HTTPS is the encrypted version of HTTP — the little padlock in your browser's address bar. Vercel sets this up automatically for any domain you connect, including custom domains. CDN stands for Content Delivery Network — copies of your site stored on servers around the world, so visitors in Tokyo get it from a Tokyo server (fast) instead of from Vercel's headquarters (slow). Free with Vercel.
...
A quick note on the stack choice. There's a "true MDX" approach using `@next/mdx` and a simpler approach using `gray-matter` + `react-markdown` + `remark-gfm`. The simpler approach can't embed React components inside posts, but for text-heavy blogs it's plenty. Cursor picked the simpler approach when I asked it to set up blog infrastructure, and I've had zero regrets. Saves complexity. Can migrate later if needed.
...
The next post is a Sole Prop Stack entry: my current Canadian credit card stack, what I actually use, and the hacks that move the needle. If that sounds useful, the most helpful thing right now is to share this post with one person who might find it useful.
