...
import {
  formatSolePropStackDate,
  getSolePropStackPost,
  getSolePropStackSlugs,
} from "@/lib/sole-prop-stack";
...
export function generateStaticParams() {
  return getSolePropStackSlugs().map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const post = getSolePropStackPost(slug);
...
export default async function SolePropStackEntryPage({ params }: PageProps) {
  const { slug } = await params;
  const post = getSolePropStackPost(slug);
      ...
      <main className="border-b px-5 sm:px-10 py-12" style={{ borderColor: BORDER_SUBTLE }}>
        <article className="mx-auto max-w-2xl">
          ...
          <p className="mt-8 text-xs font-medium uppercase tracking-widest text-[#a8332a]">
            Sole prop stack · Post {post.entry_number}
          </p>
